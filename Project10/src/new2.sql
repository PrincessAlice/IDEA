...
6.查询
select b.id,b.name book_name,b.author,b.price,c.name category_name,
c.id category_id,bi.start_time,bi.end_time from book b 
join category c on b.category_id = c.id
join borrow_info bi on bi.book_id = b.id where c.name='艺术';
...
4.学生表子查询-嵌套查询
 select *from score sco where exists(select*from course cou where )